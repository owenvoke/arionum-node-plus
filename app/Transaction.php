...
final class Transaction extends Model
{
    ...
    public function reverse($block)
    {
        ...
        foreach ($r as $x) {
            ...
            if ($x['version'] == 2) {
                ...
                if ($rez != 1) {
                    Log::info('Update alias balance minus failed');
                    return false;
                }
            } else {
                ...
                if ($x['version'] != 100 && $x['version'] < 111) {
                    ...
                    if ($rez != 1) {
                        Log::info('Update accounts balance minus failed');
                        return false;
                    }
                }
            }
            ...
            if ($x['version'] >= 100 && $x['version'] < 110 && $x['height'] >= 80000) {
                if ($x['version'] == 100) {
                    ...
                    if ($rez != 1) {
                        Log::info('Delete from masternode failed');
                        return false;
                    }
    ...
    public function check($x, $height = 0)
    {
        ...
        // the value must be >=0
        if ($x['val'] < 0) {
            Log::info($x['id'].' - Value below 0');
            return false;
        }

        // the fee must be >=0
        if ($x['fee'] < 0) {
            Log::info($x['id'].' - Fee below 0');
            return false;
        }
        ...
        if ($x['version'] >= 100 && $x['version'] < 110 && $height >= 80000) {
            if ($x['version'] == 100) {
                ...
                if (!filter_var($message, FILTER_VALIDATE_IP, FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE)) {
                    Log::info('The Masternode IP is invalid');
                    return false;
                }
                global $db;
                $existing = $db->single(
        ...
        if ($x['version'] == 1) {
            ...
            if (!$acc->validAlias($x['dst'])) {
                Log::info($x['id'].' - Invalid destination alias');
                return false;
            }
        }
